(() => {
  ...
  function intelligenceBandMarkup(channel, primaryLabel, primaryValue, primaryHelp, secondaryLabel, secondaryValue, metrics) {
    return `
      <header><div><h3>${safe(channel)}</h3><p>${channel === 'Store' ? 'DMart and Reliance account decisions' : 'Blinkit, Zepto, BigBasket, Swiggy and other accounts'}</p></div><span class="channel-growth ${metrics.attention ? 'down' : ''}">${safe(metrics.headline)}</span></header>
