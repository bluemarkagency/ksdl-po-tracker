...
function missingTripOrders() {
  const linked = linkedPoIds();
  return orders.filter(order => usesEmailGrn(order.customer) && order.status === 'Delivered' && order.deliveryCompletedDate && order.deliveryCompletedDate >= GRN_TRIP_GUARD_START && !linked.has(order.id) && customerMatches(order.customer));
}
...
function renderOpenOrders() {
  ...
  $('poTableBody').innerHTML = visible.map(order => {
    ...
    const invoiceCell = invoiceReady
      ? `<div class="invoice-ready-cell"><span class="invoice-ready-badge">Invoice ready</span><strong>${safe(order.invoice)}</strong><span class="secondary-line">${dateText(order.invoiceDate)} · ${money(order.invoiceAmount)}</span><div class="inline-link-row"><button class="edit-date-button" type="button" data-open-doc="${safe(order.invoicePath)}">View / Print</button>${canPrepareInvoice() ? `<button class="edit-date-button" type="button" data-prepare-invoice="${order.id}">Edit invoice</button>` : ''}</div></div>`
      : `<div class="invoice-ready-cell"><span class="invoice-missing-badge">Not prepared</span>${order.invoice ? `<strong>${safe(order.invoice)}</strong><span class="secondary-line">Complete the missing invoice details</span>` : '<span class="secondary-line">Accountant action required</span>'}${canPrepareInvoice() ? `<button class="edit-date-button" type="button" data-prepare-invoice="${order.id}">Add invoice</button>` : ''}</div>`;
    const selection = isDispatchViewer() ? '' : `<input class="row-select" type="checkbox" data-select-po="${order.id}" ${state.selected.has(order.id) ? 'checked' : ''} aria-label="Select PO ${safe(order.po)}" />`;
    const appointmentAction = isDispatchViewer() ? '' : `<button class="edit-date-button" type="button" data-edit-appointment="${order.id}">Edit date</button>`;
    const editAction = isDispatchViewer() ? '' : `<button class="edit-date-button" type="button" data-edit-po="${order.id}">Edit PO</button>`;
    return `<tr><td class="select-cell" data-label="Select">${selection}</td><td class="po-identity-cell" data-label="PO / Customer"><span class="po-number">${safe(order.po)}</span><span class="row-customer">${customerLogo(order.customer)}${safe(order.customer)}</span></td><td data-label="PO date">${dateText(order.poDate)}</td><td data-label="Status"><span class="status-pill ${statusClass(order.status)}">${safe(order.status)}</span></td><td data-label="Appointment"><div class="appointment-cell">${appointmentMarkup(order)}${appointmentAction}</div></td><td data-label="Location">${safe(order.location)}</td><td class="value-cell" data-label="PO value">${money(order.value)}</td><td data-label="Invoice">${invoiceCell}</td><td data-label="Action"><div class="row-action-links"><button class="edit-date-button" type="button" data-view-po="${order.id}">View PO</button>${editAction}</div></td></tr>`;
  }).join('');
...
function renderMissingTripExceptions() {
  const exceptions = missingTripOrders();
  $('missingTripCard').classList.toggle('hidden', exceptions.length === 0); $('missingTripCount').textContent = `${exceptions.length} action${exceptions.length === 1 ? '' : 's'}`;
  $('missingTripList').innerHTML = exceptions.map(order => `<article class="missing-trip-row"><div><span class="exception-label">GRN RECEIVED · TRIP NOT CREATED</span><strong>PO ${safe(order.po)} · ${safe(order.customer)}</strong><small>${safe(order.location)} · Delivered ${dateText(order.deliveryCompletedDate)}${order.invoice ? ` · Invoice ${safe(order.invoice)}` : ''}</small></div><div class="missing-trip-actions"><button class="edit-date-button" type="button" data-view-po="${order.id}">View PO</button>${isDispatchViewer() ? '' : `<button class="primary-mini" type="button" data-record-missing-trip="${order.id}">Record missed trip</button>`}</div></article>`).join('');
}
...
function renderTrips() {
  ...
  $('tripList').innerHTML = visible.map(trip => {
    ...
    let actions = '';
    if (!isDispatchViewer() && trip.status === 'Awaiting GRN' && !correction) actions = `<button type="button" data-edit-trip="${trip.id}">Edit</button><span class="grn-waiting">Waiting for customer GRN</span><button class="danger" type="button" data-delete-trip="${trip.id}">Delete</button>`;
    else if (!isDispatchViewer() && (ACTIVE_TRIP_STATUSES.has(trip.status) || correction)) actions = `<button type="button" data-edit-trip="${trip.id}">Edit</button><button class="primary-mini" type="button" data-complete-trip="${trip.id}">${correction ? 'Correct delivery' : 'Complete'}</button><button class="danger" type="button" data-delete-trip="${trip.id}">Delete</button>`;
...
function planCard(order, link = null, allowMissingInvoice = false) {
  const invoicePath = link?.invoicePath || order.invoicePath || '';
  const invoiceDocumentField = invoicePath
    ? `<div class="trip-invoice-ready"><div><span class="invoice-ready-badge">Invoice ready</span><small>Saved invoice will be linked to this trip automatically.</small></div>${docButton(invoicePath, 'View / Print invoice')}</div>`
    : `<label class="trip-invoice-upload">Invoice copy${allowMissingInvoice ? ' (optional)' : '*'}<input data-plan-file type="file" accept="application/pdf" /><small class="uat-file-note">Upload the original Tally PDF when it is available.</small></label>`;
  const required = allowMissingInvoice ? '' : '*';
  return `<article class="uat-po-plan" data-plan-po="${order.id}" data-link-id="${safe(link?.id || '')}" data-existing-file="${safe(invoicePath)}" data-invoice-state="idle" data-invoice-items="[]"><div class="uat-po-plan-head"><div><strong>PO ${safe(order.po)} · ${safe(order.customer)}</strong><span>${safe(order.location)} · ${money(order.value)}</span></div>${appointmentMarkup(order)}</div><div class="uat-po-fields"><label>Invoice number${required}<input data-plan-invoice value="${safe(link?.invoice || order.invoice || '')}" placeholder="Tally invoice number" /></label><label>Invoice date${required}<input data-plan-invoice-date type="date" value="${safe(link?.invoiceDate || order.invoiceDate || '')}" /></label><label>Invoice amount (₹)${required}<input data-plan-invoice-amount type="number" min="0" step="0.01" value="${Number(link?.invoiceAmount || order.invoiceAmount || 0) || ''}" /></label><label>Allocated cost (₹)<input data-plan-cost type="number" min="0" step="0.01" value="${Number(link?.allocatedCost || 0) || ''}" placeholder="Optional" /></label>${invoiceDocumentField}</div><div class="invoice-read-status"></div></article>`;
}
function openCreateTrip() {
  const selected = orders.filter(order => state.selected.has(order.id)); if (!selected.length) return;
  state.editTripId = null; state.missingTripOrderId = null; $('tripError').textContent = ''; $('tripDialogTitle').textContent = 'Create a new trip'; $('tripSummary').textContent = `${selected.length} PO${selected.length === 1 ? '' : 's'} selected for this vehicle.`; $('tripDialogNote').textContent = 'Invoices prepared against Open POs are filled automatically. Staff can view or print them and only need to upload a file when an invoice was not prepared or must be replaced.'; $('tripDate').value = todayIso(); $('tripTransporter').innerHTML = transporterOptions(); $('tripVehicle').value = ''; $('tripDriver').value = ''; $('tripPhone').value = ''; $('tripCost').value = ''; $('tripPoList').innerHTML = selected.map(order => planCard(order)).join(''); $('tripDialog').showModal();
}
function openRecordMissingTrip(id) {
  const order = orders.find(item => item.id === id); if (!order || !missingTripOrders().some(item => item.id === id)) return;
  state.editTripId = null; state.missingTripOrderId = id; $('tripError').textContent = ''; $('tripDialogTitle').textContent = 'Record missed dispatch'; $('tripSummary').textContent = `Customer GRN confirms PO ${order.po} was delivered on ${dateText(order.deliveryCompletedDate)}. Add the transport details to complete the dispatch record.`; $('tripDialogNote').textContent = 'This creates a completed historical trip only. The PO remains Delivered; it will not return to the Open PO list.'; $('tripDate').value = order.deliveryCompletedDate || todayIso(); $('tripTransporter').innerHTML = transporterOptions(); $('tripVehicle').value = ''; $('tripDriver').value = ''; $('tripPhone').value = ''; $('tripCost').value = ''; $('tripPoList').innerHTML = planCard(order, null, true); $('tripDialog').showModal();
}
function openEditTrip(id) {
  const trip = trips.find(item => item.id === id); if (!trip) return;
  state.editTripId = id; state.missingTripOrderId = null; $('tripError').textContent = ''; $('tripDialogTitle').textContent = 'Edit trip'; $('tripSummary').textContent = `${trip.pos.length} linked PO${trip.pos.length === 1 ? '' : 's'}.`; $('tripDialogNote').textContent = 'Invoices prepared against Open POs are filled automatically. Staff can view or print them and only need to upload a file when an invoice was not prepared or must be replaced.';
...
async function saveTrip(event) {
  ...
  try {
    ...
    if (editTrip) {
      ...
      const completedDate = missingTripOrder?.deliveryCompletedDate || $('tripDate').value;
      await api('/rest/v1/delivery_trips', { method: 'POST', headers: { 'Content-Type': 'application/json', Prefer: 'return=minimal' }, body: JSON.stringify({ id: tripId, status: missingTripOrder ? 'Delivered' : 'Dispatched', ...payload }) });
      await api('/rest/v1/delivery_trip_pos', { method: 'POST', headers: { 'Content-Type': 'application/json', Prefer: 'return=minimal' }, body: JSON.stringify(details.map(detail => ({ trip_id: tripId, purchase_order_id: detail.purchaseOrderId, allocation_method: 'Manual', allocated_cost: detail.allocatedCost, invoice_number: detail.invoiceNumber || null, invoice_date: detail.invoiceDate || null, invoice_amount: detail.invoiceAmount || 0, invoice_attachment_url: detail.invoicePath || null, delivery_status: missingTripOrder ? 'Delivered' : 'Pending', delivered_at: missingTripOrder ? `${completedDate}T12:00:00+05:30` : null }))) }); state.selected.clear();
    }
    ...
    $('tripDialog').close(); toast(editTrip ? 'Trip updated.' : missingTripOrder ? `Missed dispatch recorded for PO ${missingTripOrder.po}.` : 'Trip created. Selected POs moved to POs in Trip.'); state.missingTripOrderId = null; await loadData(); $('tripsTitle').scrollIntoView({ behavior: 'smooth', block: 'start' });
...
$('selectAll').addEventListener('change', event => { if (isDispatchViewer()) return; filteredAvailableOrders().forEach(order => event.target.checked ? state.selected.add(order.id) : state.selected.delete(order.id)); renderOpenOrders(); });
$('poTableBody').addEventListener('change', event => { if (isDispatchViewer() || !event.target.matches('[data-select-po]')) return; event.target.checked ? state.selected.add(event.target.dataset.selectPo) : state.selected.delete(event.target.dataset.selectPo); renderOpenOrders(); });
