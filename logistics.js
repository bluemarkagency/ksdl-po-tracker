...
function renderOpenOrders() {
  ...
  $('poTableBody').innerHTML = visible.map(order => {
    ...
    const invoiceCell = invoiceReady
      ? `<div class="invoice-ready-cell"><span class="invoice-ready-badge">Invoice ready</span><strong>${safe(order.invoice)}</strong><span class="secondary-line">${dateText(order.invoiceDate)} · ${money(order.invoiceAmount)}</span></div>`
...
function openManagePo(id) {
  const order = orders.find(item => item.id === id); if (!order) return;
  const invoiceReady = Boolean(order.invoice && order.invoiceDate && order.invoiceAmount > 0 && order.invoicePath);
  $('managePoTitle').textContent = `PO ${order.po}`; $('managePoSummary').textContent = `${order.customer} · ${order.location} · ${money(order.value)}`;
  $('managePoActions').innerHTML = `
    <button type="button" data-view-po="${order.id}"><span>View PO details</span><small>PO copy, appointment and delivery information</small></button>
    <button type="button" data-edit-po="${order.id}"><span>Edit PO</span><small>Correct PO details, status or location</small></button>
    ${canPrepareInvoice() ? `<button class="invoice-action" type="button" data-prepare-invoice="${order.id}"><span>${invoiceReady ? 'Update prepared invoice' : 'Prepare invoice'}</span><small>${invoiceReady ? 'Replace or correct the saved Tally invoice' : 'Attach the Tally invoice before dispatch'}</small></button>` : ''}
    ${order.invoicePath ? `<button class="print-action" type="button" data-open-doc="${safe(order.invoicePath)}"><span>View / Print invoice</span><small>${safe(order.invoice || 'Saved invoice copy')}</small></button>` : ''}`;
  $('managePoDialog').showModal();
}
...
document.body.addEventListener('click', event => { const manageAction = event.target.closest('#managePoDialog [data-view-po], #managePoDialog [data-edit-po], #managePoDialog [data-prepare-invoice], #managePoDialog [data-open-doc]'); if (manageAction) $('managePoDialog').close(); const managePo = event.target.closest('[data-manage-po]'); if (managePo) openManagePo(managePo.dataset.managePo); const po = event.target.closest('[data-view-po]'); if (po) showPo(po.dataset.viewPo); const appointment = event.target.closest('[data-edit-appointment]'); if (appointment) openAppointmentEdit(appointment.dataset.editAppointment); const editPo = event.target.closest('[data-edit-po]'); if (editPo) openEditPo(editPo.dataset.editPo); const prepareInvoice = event.target.closest('[data-prepare-invoice]'); if (prepareInvoice) openPrepareInvoice(prepareInvoice.dataset.prepareInvoice); const edit = event.target.closest('[data-edit-trip]'); if (edit) openEditTrip(edit.dataset.editTrip); const complete = event.target.closest('[data-complete-trip]'); if (complete) openCompleteTrip(complete.dataset.completeTrip); const remove = event.target.closest('[data-delete-trip]'); if (remove) deleteTrip(remove.dataset.deleteTrip); const documentButton = event.target.closest('[data-open-doc]'); if (documentButton) openDocument(documentButton.dataset.openDoc); });
