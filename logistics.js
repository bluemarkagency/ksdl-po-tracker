...
function renderOpenOrders() {
  ...
  $('poTableBody').innerHTML = visible.map(order => {
    ...
    return `<tr><td class="select-cell" data-label="Select"><input class="row-select" type="checkbox" data-select-po="${order.id}" ${state.selected.has(order.id) ? 'checked' : ''} aria-label="Select PO ${safe(order.po)}" /></td><td class="po-identity-cell" data-label="PO / Customer"><span class="po-number">${safe(order.po)}</span><span class="row-customer">${customerLogo(order.customer)}${safe(order.customer)}</span></td><td data-label="PO date">${dateText(order.poDate)}</td><td data-label="Status"><span class="status-pill ${statusClass(order.status)}">${safe(order.status)}</span></td><td data-label="Appointment"><div class="appointment-cell">${appointmentMarkup(order)}<button class="edit-date-button" type="button" data-edit-appointment="${order.id}">Edit date</button></div></td><td data-label="Location">${safe(order.location)}</td><td class="value-cell" data-label="PO value">${money(order.value)}</td><td data-label="Invoice">${invoiceCell}</td><td data-label="Action"><div class="row-action-links"><button class="edit-date-button" type="button" data-view-po="${order.id}">View PO</button><button class="edit-date-button" type="button" data-edit-po="${order.id}">Edit PO</button></div></td></tr>`;
  }).join('');
...
function renderAll() { renderCustomerSwitcher(); renderKpis(); renderSchedule(); renderMissingTripExceptions(); renderOpenOrders(); renderTrips(); }
